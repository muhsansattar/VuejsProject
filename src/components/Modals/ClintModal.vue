...
const tableData = dataStore.clientData;


const selectclient = (id:number)=>{
    if(id){
      dataStore.selectClient(id);
    }
...
<template>
  <div
    ...
    <div class="mx-12 overflow-x-auto">
      <table class="w-full text-xs sm:text-xs rtl:text-right text-black dark:text-white">
        <!-- //////// Table Heading ////////// -->
        ...
        <thead class="text-base font-extrabold">
          <tr class="border-b border-black text-center border-opacity-10">
            ...
            <th scope="col" class="px-6 py-0 text-white dark:text-extraGrey">
              Transferencia
            </th>
          </tr>
        </thead>
        <!-- ////////////// Table Body ///////////// -->
        <tbody>
          <tr v-if="filteredTableData.length > 0" v-for="item in filteredTableData" :key="item.id"
            ...
            <td  class="px-6">
              <img @click="selectclient(item.id)" src="@/assets/images/user.png" class="h-6 mx-auto" />
            </td>
          </tr>
          <tr v-else
